...
class ApiController extends Controller {
     ...
     */
    public function save(string $folder, string $fileName, string $content): DataResponse {
        try {
            $this->logger->info("Saving file to folder: $folder, user: {$this->userId}");
    
            // Get the user's Nextcloud folder
            $userFolder = $this->rootFolder->getUserFolder($this->userId);
            ...
            // ✅ Use provided filename (or default to 'metadata.xml')
            $fileName = empty($fileName) ? 'metadata.xml' : $fileName;
    
            // ✅ Ensure filename is safe (remove special characters)
            $fileName = preg_replace('/[^a-zA-Z0-9_\-.]/', '_', $fileName);
            if (!str_ends_with($fileName, '.xml')) {
                $fileName .= '.xml'; // Ensure .xml extension
            }
    
            // ✅ Check if file exists and create a unique version
            if ($targetFolder->nodeExists($fileName)) {
                $timestamp = date('Ymd_His');
                $fileName = pathinfo($fileName, PATHINFO_FILENAME) . "_$timestamp.xml";
            }
            ...
            return new DataResponse(['message' => "File saved successfully as $fileName"]);
        } catch (\Exception $e) {
